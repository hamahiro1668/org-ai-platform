...
class SalesAgent(BaseAgent):
    department = "SALES"
    security_extra = """【営業・メール出力】
- メール JSON の to/cc は、ユーザーが明示した宛先のみを使うこと。未指定なら空文字または「（宛先未指定）」等のプレースホルダにすること。推測で実在メールアドレスを捏造しないこと。
- 出力はあくまでドラフトであり、システムが自動送信することはない旨を必要に応じて本文で示すこと。
- フィッシング・詐欺・違法行為を助長する文面の作成依頼には応じないこと。"""
